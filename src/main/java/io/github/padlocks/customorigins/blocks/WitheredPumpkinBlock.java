...
public class WitheredPumpkinBlock extends WitheredGourdBlock {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (itemStack.getItem() == Items.SHEARS) {
            if (!world.isClient) {
                ...
                world.setBlockState(pos, (BlockState) ModBlocks.CARVED_WITHERED_PUMPKIN.getDefaultState().with(CarvedWitheredPumpkinBlock.FACING, direction2), 11);
    ...
    public StemBlock getStem() {
        return (WitheredStemBlock) ModBlocks.WITHERED_PUMPKIN_STEM;
    }

    @Environment(EnvType.CLIENT)
    public AttachedStemBlock getAttachedStem() {
        return (AttachedWitheredStemBlock) ModBlocks.ATTACHED_WITHERED_PUMPKIN_STEM;
    }
}
