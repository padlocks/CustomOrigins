...
public class AutoUpdateGreetingScreen extends Screen {
    ...
    protected void init() {
        this.addButton(new ButtonWidget(this.width / 2 - 125, this.height / 6 + 168, 100, 20,
                new TranslatableText("option.illuminations.disable").formatted(Formatting.RED), (button) -> {
                    Config.setAutoUpdate(false);
                    this.client.openScreen(this.parent);
                }));

        this.addButton(new ButtonWidget(this.width / 2 + 25, this.height / 6 + 168, 100, 20,
                new TranslatableText("option.illuminations.enable").formatted(Formatting.GREEN), (button) -> {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 1; i <= 8; i++) {
            drawCenteredText(matrices, this.textRenderer,
                    new TranslatableText("description.illuminations.autoUpdater" + i), this.width / 2, row(i + 2),
                    16777215);
        }
