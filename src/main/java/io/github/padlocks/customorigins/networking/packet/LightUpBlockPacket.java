package io.github.padlocks.customorigins.networking.packet;

import io.github.padlocks.customorigins.impl.mixin.AbstractFurnaceBlockEntityAccessor;
...
public class LightUpBlockPacket {
    public static final Identifier ID = new Identifier(CustomOriginsMod.MOD_ID, "light_up_block");
    ...
    public static void handle(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler network,
        ...
        server.execute(() -> {
            ...
            if (state.getBlock() instanceof AbstractFurnaceBlock || state.getBlock() instanceof CampfireBlock) {
                ...
                ((ServerWorld) player.world).spawnParticles(particleEffect, pos.getX() + 0.5, pos.getY() + 0.3,
                        pos.getZ() + 0.5, particleCount, player.getRandom().nextDouble() * 0.2D - 0.1D, 0.1D,
                        player.getRandom().nextDouble() * 0.2D - 0.1D, 0.05D);
